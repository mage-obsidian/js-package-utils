...
describe("generateInterceptors", () => {
    ...
    test("should generate interceptors correctly", async () => {
        ...
        expect(source).toContain(
            `import * as originalModule from ${JSON.stringify(`/@fs${targetModulePath}?originalIntercepted`)};`,
        );
        expect(source).toContain(
            `import * as interceptor_0 from ${JSON.stringify(`/@fs${pluginModulePath}`)};`,
        );
        expect(source).toContain(
            `interceptorManager.addInterceptor("${targetIdentifier}::targetFunction", "TestPlugin", "before", interceptor_0.beforeTargetFunction, 10);`,
        );
        expect(source).toContain(
            `interceptorManager.addInterceptor("${targetIdentifier}::default", "TestPlugin", "before", interceptor_0.beforeDefault, 10);`,
        );
    ...
    test("should handle missing target module gracefully", async () => {
        ...
        const result =
            await generateInterceptorsService.generateInterceptors("Vendor/theme-missing");
    ...
    test("should throw error if plugin targets non-existent method", async () => {
        ...
        await expect(generateInterceptorsService.generateInterceptors(themeName)).rejects.toThrow(
            /does not export/,
        );
    });
